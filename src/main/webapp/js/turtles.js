...
Turtles.Viz.Prices.query = function() {
    ...
    query.setQuery("select B, min(F) group by B pivot D order by B options no_format");
...
Turtles.Viz.Prices.dashboard = function(response) {
    ...
    var dashboard = new google.visualization.Dashboard(document.getElementById("coe-prices"));

    var motorcycleDashboard = new google.visualization.Dashboard(document.getElementById("coe-prices-mc"));
...
Turtles.Viz.Quotas.query = function() {
    ...
    query.setQuery("select B, C, D, E, G, H, F order by B label D 'Category', C 'Year', F 'Price' options no_format");
...
Turtles.Viz.QuotasYearly.query = function() {
    ...
    query.setQuery("select C, D, sum(E), max(F), avg(F) group by C, D label D 'Category', C 'Year', sum(E) 'Yearly Quota', max(F) 'Highest Price', avg(F) 'Average Price' options no_format");
...
Turtles.Viz.BidDistribution.query = function() {
    ...
    query.setQuery("select A, C, H, J, L where L > 0 label A 'Bidding Exercise', C 'Category', L 'Number Of Successful Bids' options no_format");
...
Turtles.Viz.BidDistribution.dashboard = function(response) {
    ...
    // Upper limit of bid range

    MoneyFormat.format(data, 3);

    // #Successful bids

    IntegerFormat.format(data, 4);
...
Turtles.Viz.Overbidding.query = function() {
    ...
    query.setQuery("select A, C, sum(L) where L > 0 and (M = 'Total number of successful bids' or M = 'Bids exceeding QP by 10% or more') and C != 'Category D (Motorcycles)' group by A, C pivot M label A 'Bidding Exercise', C 'Category', sum(L) '' options no_format");
...
Turtles.Viz.Overbidding.dashboard = function(response) {
    ...
    // #Bids exceeding QP by 10% or more
...
Turtles.Viz.Subscription.query = function() {
    ...
    query.setQuery("select B, G/E, F, C, D order by B label G/E 'Subscription Rate', D 'Category', C 'Year', F 'Price' options no_format");
...
Turtles.Viz.Population.Cars.query = function() {
    ...
    query.setQuery("select A, B, C, D, E, F, G, H, I, J, K, L order by A label A 'Brand' options no_format");
